...
The distinction matters: when a mutation succeeds, Covenant automatically refetches all listeners watching the same resources.
...
Context is per-request data (typically auth). Derivation is shared utilities derived from context.
...
The `derivation` function runs once per request. Use it to consolidate auth guards, database access, and other shared logic.
